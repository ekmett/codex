...
instance IsObject Blob where
  type Rec Blob = Blob_
  _reference b = [U.exp|hb_blob_t * { hb_blob_reference($blob:b) }|]
  _destroy b = [C.block|void { hb_blob_destroy($blob:b); }|]
  _get_user_data b k = [U.exp|void * { hb_blob_get_user_data($blob:b,$key:k) }|]
...
instance IsObject Buffer where
  ...
  _reference b = [U.exp|hb_buffer_t * { hb_buffer_reference($buffer:b) }|]
  _destroy b = [C.block|void { hb_buffer_destroy($buffer:b); }|]
  _get_user_data b k = [U.exp|void * { hb_buffer_get_user_data($buffer:b,$key:k) }|]
...
instance IsObject Face where
  type Rec Face = Face_
  _reference b = unsafeIOToPrim [U.exp|hb_face_t * { hb_face_reference($face:b) }|]
  _destroy b = unsafeIOToPrim [C.block|void { hb_face_destroy($face:b); }|]
  _get_user_data b k = [U.exp|void * { hb_face_get_user_data($face:b,$key:k) }|]
...
instance IsObject Font where
  type Rec Font = Font_
  _reference b = [U.exp|hb_font_t * { hb_font_reference($font:b) }|]
  _destroy b = [C.block|void { hb_font_destroy($font:b); }|]
  _get_user_data b k = [U.exp|void * { hb_font_get_user_data($font:b,$key:k) }|]
...
instance IsObject FontFuncs where
  ...
  _reference b = [U.exp|hb_font_funcs_t * { hb_font_funcs_reference($font-funcs:b) }|]
  _destroy b = [C.block|void { hb_font_funcs_destroy($font-funcs:b); }|]
  _get_user_data b k = [U.exp|void * { hb_font_funcs_get_user_data($font-funcs:b,$key:k) }|]
...
instance IsObject Map where
  type Rec Map = Map_
  _reference b = [U.exp|hb_map_t * { hb_map_reference($map:b) }|]
  _destroy b = [C.block|void { hb_map_destroy($map:b); }|]
  _get_user_data b k = [U.exp|void * { hb_map_get_user_data($map:b,$key:k) }|]
...
instance IsObject Set where
  type Rec Set = Set_
  _reference b = [U.exp|hb_set_t * { hb_set_reference($set:b) }|]
  _destroy b = [C.block|void { hb_set_destroy($set:b); }|]
  _get_user_data b k = [U.exp|void * { hb_set_get_user_data($set:b,$key:k) }|]
...
instance IsObject ShapePlan where
  ...
  _reference uf = [U.exp|hb_shape_plan_t * { hb_shape_plan_reference($shape-plan:uf) }|]
  _destroy uf = [C.block|void { hb_shape_plan_destroy($shape-plan:uf); }|]
  _get_user_data b k = [U.exp|void * { hb_shape_plan_get_user_data($shape-plan:b,$key:k) }|]
...
instance IsObject UnicodeFuncs where
  ...
  _reference uf = [U.exp|hb_unicode_funcs_t * { hb_unicode_funcs_reference($unicode-funcs:uf) }|]
  _destroy uf = [C.block|void { hb_unicode_funcs_destroy($unicode-funcs:uf); }|]
  _get_user_data b k = [U.exp|void * { hb_unicode_funcs_get_user_data($unicode-funcs:b,$key:k) }|]
