...
buffer_create = unsafeIOToPrim $ [U.exp|hb_buffer_t * { hb_buffer_create() }|] >>= foreignBuffer
...
buffer_diff buffer reference dottedcircle_glyph (fromIntegral -> position_fuzz) = unsafeIOToPrim
  [U.exp|hb_buffer_diff_flags_t { hb_buffer_diff($buffer:buffer,$buffer:reference,$(hb_codepoint_t dottedcircle_glyph),$(unsigned int position_fuzz)) }|]
...
buffer_reset b = unsafeIOToPrim [U.block|void { hb_buffer_reset($buffer:b); }|]
...
buffer_reverse_range b (fromIntegral -> start) (fromIntegral -> end) = unsafeIOToPrim
  [U.block|void { hb_buffer_reverse_range($buffer:b,$(unsigned int start), $(unsigned int end)); }|]

buffer_add :: PrimMonad m => Buffer (PrimState m) -> Codepoint -> Int -> m ()
buffer_add buffer codepoint (fromIntegral -> cluster) = unsafeIOToPrim
  [U.block|void { hb_buffer_add($buffer:buffer,$(hb_codepoint_t codepoint),$(unsigned int cluster)); }|]
...
buffer_append buffer source (fromIntegral -> start) (fromIntegral -> end) = unsafeIOToPrim
  [U.block|void { hb_buffer_append($buffer:buffer,$buffer:source,$(unsigned int start),$(unsigned int end)); }|]
...
buffer_get_glyph_positions b = unsafeIOToPrim $ alloca \plen -> do
  positions <- [U.exp|hb_glyph_position_t * { hb_buffer_get_glyph_positions($buffer:b,$(unsigned int * plen)) }|]
  len <- peek plen
...
buffer_get_glyph_infos b = unsafeIOToPrim $ alloca \plen -> do
  pinfos <- [U.exp|hb_glyph_info_t * { hb_buffer_get_glyph_infos($buffer:b,$(unsigned int * plen)) }|]
  len <- peek plen
  peekVector (fromIntegral len) pinfos

glyph_info_get_glyph_flags :: GlyphInfo -> GlyphFlags
glyph_info_get_glyph_flags info = [U.pure|hb_glyph_flags_t { hb_glyph_info_get_glyph_flags($glyph-info:info) }|]

buffer_guess_segment_properties :: PrimMonad m => Buffer (PrimState m) -> m ()
buffer_guess_segment_properties b = unsafeIOToPrim
  [C.block|void { hb_buffer_guess_segment_properties($buffer:b); }|]
  -- requires safe call as it can call back to our unicode_funcs for script detection

buffer_normalize_glyphs :: PrimMonad m => Buffer (PrimState m) -> m ()
buffer_normalize_glyphs b = unsafeIOToPrim [U.block|void { hb_buffer_normalize_glyphs($buffer:b); }|]
...
--
-- This is only being used by the shaper.
...
buffer_set_message_func b hfun = unsafeIOToPrim do
  (castFunPtr -> f) <- mkBufferMessageFunc \pbuffer pfont cmsg _ -> do
    buffer <- [U.exp|hb_buffer_t * { hb_buffer_reference($(hb_buffer_t * pbuffer)) }|] >>= foreignBuffer
    font <- [U.exp|hb_font_t * { hb_font_reference($(hb_font_t * pfont)) }|] >>= foreignFont
...
buffer_direction b = unsafeStateVar g s where
  g = [U.exp|hb_direction_t { hb_buffer_get_direction($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_direction($buffer:b,$(hb_direction_t v)); }|]
...
buffer_script b = unsafeStateVar g s where
  g = [U.exp|hb_script_t { hb_buffer_get_script($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_script($buffer:b,$(hb_script_t v)); }|]
...
buffer_language b = unsafeStateVar g s where
  g = Language <$> [U.exp|hb_language_t { hb_buffer_get_language($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_language($buffer:b,$language:v); }|]
...
buffer_flags b = unsafeStateVar g s where
  g = [U.exp|hb_buffer_flags_t { hb_buffer_get_flags($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_flags($buffer:b,$(hb_buffer_flags_t v)); }|]
...
buffer_cluster_level b = unsafeStateVar g s where
  g = [U.exp|hb_buffer_cluster_level_t { hb_buffer_get_cluster_level($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_cluster_level($buffer:b,$(hb_buffer_cluster_level_t v)); }|]
...
buffer_content_type b = unsafeStateVar g s where
  g = [U.exp|hb_buffer_content_type_t { hb_buffer_get_content_type($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_content_type($buffer:b,$(hb_buffer_content_type_t v)); }|]
...
buffer_segment_properties b = unsafeStateVar g s where
  g = alloca \props -> [U.block|void { hb_buffer_get_segment_properties($buffer:b,$(hb_segment_properties_t * props)); }|] *> peek props
  s v = with v \props -> [U.block|void { hb_buffer_set_segment_properties($buffer:b,$(const hb_segment_properties_t * props)); }|]
...
buffer_unicode_funcs b = unsafeStateVar g s where
  g = [U.exp|hb_unicode_funcs_t * { hb_buffer_get_unicode_funcs($buffer:b) }|] >>= foreignUnicodeFuncs
  s v = [U.block|void { hb_buffer_set_unicode_funcs($buffer:b,$unicode-funcs:v); }|]

buffer_invisible_glyph :: Buffer s -> StateVar s Codepoint
buffer_invisible_glyph b = unsafeStateVar g s where
  g = [U.exp|hb_codepoint_t { hb_buffer_get_invisible_glyph($buffer:b) }|]
  s v = [U.block|void { hb_buffer_set_invisible_glyph($buffer:b,$(hb_codepoint_t v)); }|]
...
buffer_replacement_codepoint b = unsafeStateVar g s where
  g = [U.exp|hb_codepoint_t { hb_buffer_get_replacement_codepoint($buffer:b) }|] <&> fromEnum
  s (toEnum -> v) = [U.block|void { hb_buffer_set_replacement_codepoint($buffer:b,$(hb_codepoint_t v)); }|]
